...
export default class NewZealand extends Component{
    constructor(props){
        super(props)
        this.state ={
            Newzealand:[]
    ...
    componentDidMount(){
     axios.get("https://cors-anywhere.herokuapp.com/http://newsfeedbuzz.herokuapp.com/nz/topstories")
     .then(response => {
         console.log(response.data)
         this.setState({Newzealand: response.data})
     })
     .catch((error)=>{
         console.log(error)
     })
    ...
    render(){
        let articleList = this.state.Newzealand.map((article,index) => {
            return (
